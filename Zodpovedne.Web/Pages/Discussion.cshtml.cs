...
public class DiscussionModel : BasePageModel
{
    ...
    public bool CanLikeDiscussion =>
        Discussion != null && IsUserLoggedIn &&
        Discussion.AuthorId != CurrentUserId; // Uivatel nemùe lajkovat své vlastní diskuze
    ...
    public async Task<IActionResult> OnPostAddCommentAsync()
    {
        ...
        {
            _logger.Log("Nepodaøilo se vloit komentáø, pøekroèila se maximální velikost");
            ErrorMessage = "Omlouváme se, ale komentáø se nepodaøilo vloit, pøekroèili jste maximální velikost 500 znakù.";
            return Page();
        }
    ...
    public async Task<IActionResult> OnPostDeleteDiscussionAsync()
    {
        ...
        {
            _logger.Log($"Uivatel {CurrentUserId} nemá oprávnìní na smazání diskuze {basicDiscussionInfoDto.Id}: {basicDiscussionInfoDto.Title}");
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            _logger.Log($"Nepodaøilo se odeslat poadavek na smazání diskuze dle Id {basicDiscussionInfoDto.Id}");
